...
const CancelarConsulta = ({
	...
	const consultaMutation = trpc.consulta.delete.useMutation();
	...
	const handleDelete = async () => {
		try {
			setLoader(true);
			await consultaMutation.mutateAsync({ id: content.id });
			toast.success("Cancelada com sucesso!");
			await contextUtils.consulta.allByPacienteId.refetch({ pacienteId: paciente?.paciente?.id! });
			setOpen(false);
		} catch (error) {
				...
				<div className="top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2 relative w-full max-w-2xl h-auto">
					...
					<div className="bg-white rounded-lg shadow-md relative dark:bg-black m-2 dark:border dark:border-zinc-500">
						...
						<div className="leading-relaxed p-6 space-y-6">
							<span>Gostaria de cancelar está consulta no dia {content.date}?</span>
